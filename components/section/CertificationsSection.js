			...
			<SectionLayout className={className} title={"Certifications"} dataAosDelay={dataAosDelay}>
				{
					certifications.map((certification, index) => {
						return (
								<div className={"relative mb-3 pb-3 " + (index !== (certifications.length - 1) ? "border-bottom" : '')}
								     key={index}>
									<div className={"d-flex gap-1"}>
										<i className={"la la-2x la-school"}></i>
										<div>
											<h3 className={"lh-28"}>{certification.title}</h3>
											<h3 className={"fw-600 lh-28"}>{certification.institution}</h3>
											<h4 className={"mb-3"}>Issued: {certification.issueDate}{!empty(certification?.expireDate)? `  -  ${certification?.expireDate}` : ''}</h4>
